...
class OpenAIReviewerClient(BaseReviewerClient):
    ...
    def evaluate(self, original_text, ollama_output, task_type):
        ...
        return {
            "overall_score": 4.5,
            "comment": f"Mock OpenAI review for {task_type}: {ollama_output[:20]}...",
        }
...
class GeminiReviewerClient(BaseReviewerClient):
    ...
    def evaluate(self, original_text, ollama_output, task_type):
        ...
        return {
            "overall_score": 4.2,
            "comment": f"Mock Gemini review for {task_type}: {ollama_output[:20]}...",
        }
...
class DeepSeekReviewerClient(BaseReviewerClient):
    ...
    def evaluate(self, original_text, ollama_output, task_type):
        ...
        return {
            "overall_score": 3.8,
            "comment": f"Mock DeepSeek review for {task_type}: {ollama_output[:20]}...",
        }
